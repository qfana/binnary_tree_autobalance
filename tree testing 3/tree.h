...
private:
    ...
    Node* getNextInLevelOrder(Node* current) const;
    Node* getPrevInLevelOrder(Node* current) const;
    std::vector<Node*> generateLevelOrder() const;
    Node* findNode(const T& value) const;
    
    // Методы проверки свойств
    bool isBST(Node* node, T min, T max) const;
    bool hasNoDoubleRed(Node* node) const;
...
public:
    ...
    // Методы для конструктора из файла
    void insertFromConstructor(const T& value);
...
// Включаем определение итераторов
